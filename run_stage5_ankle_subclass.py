...
def main():
    ...
    # 6. Save results
    output_dir = "results/latest/stage5_ankle_subclass"
    ...
    cm_df.to_csv(os.path.join(output_dir, "ankle_subclass_confusion_matrix.csv"))
    
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm_df, annot=True, fmt='d', cmap='Oranges')
    plt.title('Ankle Sub-classification (Top 4 Diseases)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "ankle_subclass_confusion_matrix.svg"))
    plt.close()
