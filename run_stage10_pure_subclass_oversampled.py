...
def train_and_eval_ensemble(df, unique_labels, label_map, subjects, rf_feature_cols, X_rf, y_rf, device, logger):
    ...
    for train_idx, val_idx in gkf.split(df, groups=subjects):
        ...
        criterion = nn.CrossEntropyLoss() # Weights handled by sampler
...
def main():
    ...
    # Needs GRF_metadata for CLASS_LABEL_DETAILED
    ...
    # Trial cap (max 10 trials per subject)
    ...
    out_dir = Path("results/latest/stage10_pure_subclass_oversampled")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    # Pure classes definition
    pure_classes = {
        'Hip': ['H_F', 'H_C', 'H_P'],
        'Knee': ['K_F', 'K_R', 'K_P'],
        'Ankle': ['A_F', 'A_L', 'A_R', 'A_O']
    }
    
    for joint, target_classes in pure_classes.items():
        logger.info(f"\n========== Sub-classification: {joint} (Oversampled) ==========")
        
        # Filter data for this joint
        joint_df = df[df['CLASS_LABEL_DETAILED'].isin(target_classes)].copy()
        ...
        plt.title(f'{joint} Pure Subclass Confusion Matrix (Macro F1: {f1_score_val:.2f})')
        plt.tight_layout()
        plt.savefig(out_dir / f"{joint.lower()}_cm.svg")
        plt.close()
        
        # Save classification report
        report = classification_report(trues, preds, target_names=unique_labels, output_dict=True)
        pd.DataFrame(report).transpose().to_csv(out_dir / f"{joint.lower()}_classification_report.csv")
