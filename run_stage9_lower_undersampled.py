...
def main():
    ...
    out_dir = Path("results/latest/stage9_lower_undersampled")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    # ==========================================
    # STAGE 4: Ankle vs Calcaneus (Lower Subclass) - UNDERSAMPLED
    # ==========================================
    logger.info("\n========== STAGE 4: Ankle vs Calcaneus (Lower Subclass) UNDERSAMPLED ==========")
    df_s3 = df[df['label'].isin(['Ankle', 'Calcaneus'])].copy()
    
    # --- Undersample Ankle subjects to match Calcaneus subjects ---
    calcaneus_subjects = df_s3[df_s3['label'] == 'Calcaneus']['subject_id'].unique()
    ankle_subjects = df_s3[df_s3['label'] == 'Ankle']['subject_id'].unique()
    
    # Set random seed for reproducibility
    np.random.seed(42)
    sampled_ankle_subjects = np.random.choice(ankle_subjects, size=len(calcaneus_subjects), replace=False)
    
    # Keep all calcaneus and the sampled ankle subjects
    subjects_to_keep = np.concatenate([calcaneus_subjects, sampled_ankle_subjects])
    df_s3 = df_s3[df_s3['subject_id'].isin(subjects_to_keep)].copy()
    
    logger.info(f"After Undersampling - Total trials: {len(df_s3)}")
    logger.info(f"Class distribution:\n{df_s3['label'].value_counts()}")
