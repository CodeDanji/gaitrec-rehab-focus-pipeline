...
def main():
    ...
    # ==========================================
    # STAGE 1: Upper vs Lower
    # ==========================================
    logger.info("\n========== STAGE 1: Upper vs Lower ==========")
    df_s1 = df[df['label'] != 'Healthy'].copy()
    
    def map_upper_lower(lbl):
        if lbl in ['Hip', 'Knee']: return 'Upper'
        if lbl in ['Ankle', 'Calcaneus']: return 'Lower'
        return 'Unknown'
        
    df_s1['label'] = df_s1['label'].apply(map_upper_lower)
    
    unique_labels_s1 = sorted(df_s1["label"].unique().tolist())
    label_map_s1 = {lbl: i for i, lbl in enumerate(unique_labels_s1)}
    logger.info(f"Stage 1 Label Map: {label_map_s1}")
    
    X_rf_s1 = df_s1[rf_feature_cols].values
    ...
    cm_df_s1 = pd.DataFrame(cm_s1, index=unique_labels_s1, columns=unique_labels_s1)
    cm_df_s1.index.name = "true_label"
    plt.figure(figsize=(6, 4))
    sns.heatmap(cm_df_s1, annot=True, fmt='d', cmap='Blues')
    ...
    # ==========================================
    # STAGE 2: Hip vs Knee (Upper Group)
    # ==========================================
    logger.info("\n========== STAGE 2: Hip vs Knee (Upper Subclass) ==========")
    df_s2 = df[df['label'].isin(['Hip', 'Knee'])].copy()
    
    unique_labels_s2 = sorted(df_s2["label"].unique().tolist())
    label_map_s2 = {lbl: i for i, lbl in enumerate(unique_labels_s2)}
    logger.info(f"Stage 2 Label Map: {label_map_s2}")
    
    X_rf_s2 = df_s2[rf_feature_cols].values
    ...
    cm_df_s2 = pd.DataFrame(cm_s2, index=unique_labels_s2, columns=unique_labels_s2)
    cm_df_s2.index.name = "true_label"
    plt.figure(figsize=(6, 4))
    sns.heatmap(cm_df_s2, annot=True, fmt='d', cmap='Oranges')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.title(f'Hip vs Knee Confusion Matrix (F1: {f1_s2:.2f})')
    plt.tight_layout()
    plt.savefig(out_dir / "s2_hip_knee_cm.svg")
    plt.close()
    ...
    plt.figure(figsize=(10, 6))
    sns.barplot(data=imp_df, x='F1_Drop', y='Feature', hue='Type', dodge=False)
    plt.title('Soft Ensemble Feature Importance (Hip vs Knee)')
    plt.xlabel('Macro F1 Drop (Baseline - Permuted)')
    plt.tight_layout()
