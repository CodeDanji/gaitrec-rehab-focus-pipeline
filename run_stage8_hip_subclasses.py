...
def main():
    ...
    # Preprocess features using common pipeline
    primary_clean_df = get_cohort_primary_clean(features)
    
    # Join with metadata to get CLASS_LABEL_DETAILED
    ...
    # Apply Trial Cap (10 trials per subject)
    df = apply_trial_cap(df)
    
    # Overwrite label with detailed subclass
    ...
    out_dir = Path("results/latest/stage8_subclasses")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    logger.info(f"\n========== STAGE 8: Hip Subclass ({', '.join(target_classes)}) ==========")
    ...
    trues, preds, f1, models_rf, models_cnn, val_datasets = train_and_eval_ensemble(
        df, unique_labels, label_map, subjects, rf_feature_cols, X_rf, y_rf, device, logger, apply_weights=True
    )
    ...
    plt.title(f'Hip Subclass Confusion Matrix (F1: {f1:.2f})')
    plt.tight_layout()
    plt.savefig(out_dir / "hip_subclass_cm.svg")
    plt.close()
    
    imp_df = calculate_importance(models_rf, models_cnn, val_datasets, rf_feature_cols, unique_labels, device, logger)
    imp_df = imp_df.sort_values(by='F1_Drop', ascending=False)
    imp_df.to_csv(out_dir / "hip_subclass_importance.csv", index=False)
    
    plt.figure(figsize=(10, 6))
    sns.barplot(data=imp_df, x='F1_Drop', y='Feature', hue='Type', dodge=False)
    plt.title('Soft Ensemble Feature Importance (Hip Subclasses)')
    plt.xlabel('Macro F1 Drop (Baseline - Permuted)')
    plt.tight_layout()
    plt.savefig(out_dir / "hip_subclass_importance.svg")
    plt.close()
